...
require(reshape2)
require(ggplot2)
source("../R/mult_factor_analysis.R")
...
comm.melt <- melt(comm.df, id.vars=c("Questions"))
print(ggplot(comm.melt, aes(x=variable, y=value, group=Questions, colour=Questions))+geom_line()+theme(axis.text.x=element_text(angle=-90))+xlab("n_factors")+ylab("communalities"))
@ 
...
print(ggplot(comm.melt, aes(x=variable, y=value))+geom_boxplot()+xlab("n_factors")+ylab("Communalities"))
@ 
